...
if($filter_form['save-filter'] == 'Filter'){
	...
	$oModel = new BSModel();
	$query = "SELECT radios.id as radio_id, radios.identificador as identificador, radios.mac as mac, eventos_alarmas.tipo as tipo, eventos_alarmas.fecha_hora as fecha_hora from eventos_alarmas left join radios on eventos_alarmas.radio_id = radios.id order by fecha_hora desc limit 70;";
	$aEvents = $oModel->Select($query);
}
